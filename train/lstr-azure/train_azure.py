#!/usr/bin/env python
# THIS IS BASED ON THE ORIGINAL TRAINING SCRIPT BUT WITH SOME EDITS TO TRAIN ON AZUREML.
...
def parse_args():
    ...
    parser.add_argument('--data-folder', type=str,
                        dest='data_folder', help='Test data folder mounting point')
...
def train(training_dbs, validation_db, start_iter=0, freeze=False):
    ...
    training_tasks = init_parallel_jobs(
        training_dbs, training_queue, sample_data)
    if val_iter:
        validation_tasks = init_parallel_jobs(
            [validation_db], validation_queue, sample_data)
...
if __name__ == "__main__":
    ...
    configs["system"]["data_dir"] = f"{data_folder}/raws"
    ...
    training_dbs = [datasets[dataset](
        configs["db"], train_split) for _ in range(threads)]
